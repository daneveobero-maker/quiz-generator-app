...
# Placeholder for Gemini API. Make sure you have the library installed:
...
# --- Core Functions (Refactored from the Tkinter class) ---

def clean_json_response(response_text):
    """
    Ensure Gemini response is a Python object (list or dict).
    Handles both JSON strings and already-parsed responses.
    """
    ...
    if isinstance(response_text, str):
        ...
        try:
            ...
            st.warning(f"JSON parsing failed. Attempting to fix common issues. Error: {e}")
            # Attempt to fix trailing commas or other minor issues if necessary,
...
def extract_text(uploaded_file):
    ...
    try:
        if file_name.endswith(".docx"):
            ...
            # PyPDF2 needs a file-like object that it can read
...
def get_exam_period(filename, term, summer_last_week_str):
    """Determines the exam period based on the week number in the filename."""
    ...
    if term == "Semester":
        ...
        try:
            ...
            if 1 <= week_num <= last_midterm_week:
                return "Midterm"
            elif (last_midterm_week + 1) <= week_num <= 17:
                return "Finals"
            else: return ""
...
def generate_prompt(q_type, mc_subtype, situational_text, difficulty, num_items, words_per_choice, body_text):
    """Dynamically generates the prompt for the Gemini API."""
    if q_type == "Multiple Choice":
        ...
        Each object must have these keys: "Subject Code", "Description", "Exam Period", "Learning Outcome", "Topic",
        "QUESTION", "Type of Question", "Taxonomy", "Choice A", "Choice B", "Choice C", "Choice D", "Answer".

        ⚠️ Rules:
        - Each choice (A, B, C, D) must be approximately {words_per_choice} words long and plausible.
        - The "Answer" must be only the letter: "A", "B", "C", or "D".
        - Do not use "None of the above" or "All of the above".
        - Do not reference the source text (e.g., "according to the text").
        - "Type of Question" must be "Multiple Choice".
        - "Taxonomy" must be one of: [Remembering, Understanding, Applying, Analyzing, Evaluating, Creating].
        - Subject code, Exam Period, Description must be empty strings.

        Text source:
        {body_text}
        """
    else: # Covers all other question types
        prompt = f"""
        ...
        Return ONLY a valid JSON array of objects, with no explanations.
        Each object must have these keys: "Subject Code", "Description", "Exam Period", "Learning Outcome", "Topic",
        "QUESTION", "Type of Question", "Taxonomy", "Answer".

        ⚠️ Rules:
        - Do NOT include multiple-choice style options (A, B, C, D).
        - For "True or False", the Answer must be only "True" or "False".
        - For "Short Answer", integrate a quote instruction like "(Integrate a quote from a famous business person like [Name])" into the question. Use a different person for each question.
        - Do not reference the source text.
...
def save_to_word(quiz_data):
    """Saves quiz data to an in-memory Word document and returns the buffer."""
    doc = docx.Document()
    ...
    for i, q in enumerate(quiz_data, 1):
        ...
        for opt in ["Choice A", "Choice B", "Choice C", "Choice D"]:
            if q.get(opt):
                doc.add_paragraph(f"    {opt[-1]}. {q[opt]}")
...
def save_to_excel(quiz_data, q_type):
    """Saves quiz data to an in-memory Excel file and returns the buffer."""
    wb = Workbook()
    ws = wb.active
    ws.title = "Quiz Questions"

    if q_type == "Multiple Choice":
        headers = ["Subject Code", "Description", "Exam Period", "Learning Outcome", "Topic",
                   "QUESTION", "Type of Question", "Taxonomy",
...
st.title("📚 Quiz Generator with Gemini - Batch Processing")
...
with st.sidebar:
    ...
    q_type = st.selectbox(
        "Select Quiz Type",
        ["Multiple Choice", "Identification", "Short Answer", "True or False", "Modified True or False"]
    )
    ...
    total_items_per_file = st.number_input("Number of Items per File (Max 50)", min_value=1, max_value=50, value=10)

    words_per_choice = st.number_input("Number of words per choice (for MC)", min_value=5, max_value=50, value=10)

    is_situational = st.checkbox("Generate Situational Questions")

    mc_subtypes_selected = []
    if q_type == "Multiple Choice":
        with st.expander("Multiple Choice Subtypes (per batch of 10)"):
...
if st.button("🚀 Generate Quiz", disabled=(not uploaded_files)):
    ...
    # --- NEW: Compile the rejection pattern once for efficiency ---
    rejection_pattern = re.compile(
        r'according to the (text|discussion|module|passage|document|lecture|source)|based on the (text|discussion|module|passage|document|lecture|source)', 
        re.IGNORECASE
    )
    ...
    with st.status("Generating Quiz...", expanded=True) as status:
        for i, file in enumerate(uploaded_files):
            ...
            if not body_text:
                ...
                # Increment progress to account for skipped calls
                current_api_call += num_loops_per_file
                progress_bar.progress(current_api_call / total_api_calls)
                continue

            topic_name = os.path.splitext(file.name)[0]
            exam_period = get_exam_period(file.name, term, summer_last_week)

            # Inner loop for API calls in batches
            for loop_num in range(num_loops_per_file):
                ...
                st.write(f"  - Calling API for batch {loop_num + 1}/{num_loops_per_file} ({questions_this_loop} questions)...")
                ...
                prompt = generate_prompt(
                    q_type, mc_subtype, situational_text, difficulty, 
                    questions_this_loop, words_per_choice, body_text
                )
                ...
                if quiz_data_batch and isinstance(quiz_data_batch, list):
                    # --- NEW: Filter out rejected questions ---
                    validated_batch = []
                    for question in quiz_data_batch:
                        question_text = question.get("QUESTION", "")
                        # Check if the question contains any of the rejection phrases
                        if rejection_pattern.search(question_text):
                            st.warning(f"Rejected question from `{file.name}` due to forbidden phrase: \"{question_text[:80]}...\"")
                        else:
                            # If the question is valid, add topic and exam period
                            question['Topic'] = topic_name
                            question['Exam Period'] = exam_period
                            validated_batch.append(question)
                    
                    all_quiz_data.extend(validated_batch) # Add only the validated questions
                else:
    ...
    if all_quiz_data:
        ...
        st.subheader("Download Your Quiz")
        ...
        with col1:
            st.download_button(
                ...
                data=excel_buffer,
                file_name="Generated_Quiz_Combined.xlsx",
        ...
        with col2:
            st.download_button(
                ...
                data=word_buffer,
                file_name="Generated_Quiz_Combined.docx",
                mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document"
            )

    else:
        st.error("No valid quiz data was generated from any of the files. Please check your files or the API response.")
